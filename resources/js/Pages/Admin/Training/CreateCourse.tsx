...
import {
  ...
  Upload, X, Save, ArrowLeft, Youtube, FileType, Image as ImageIcon
} from 'lucide-react';
...
export default function CreateCourse({ categories, skillTypes, instructors }: Props) {
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    // Build sanitized sections structure that keeps File objects for slides/document
    ...
    // Recursive helper to append nested objects/arrays and files with Laravel-compatible keys
    ...
    router.post(route('admin.training.store'), formData, {
      onSuccess: () => {
      ...
      onError: (errors) => {
        toast.dismiss(loadingToast);
        const errorMessage = Object.values(errors)[0] as string || 'Failed to save course. Please check the form.';
  ...
  const handleDocumentUpload = (sectionIndex: number, lectureIndex: number, file: File | null) => {
    ...
    // Validate file size (e.g., max 50MB for documents/videos)
        ...
        return (
          <div className="space-y-3">
            <div>
              ...
              />
              {lecture.documentUrl && (
                <p className="text-sm text-red-600 mt-1">✓ Video selected: {lecture.document?.name}</p>
              )}
            </div>
          </div>
        );
        ...
        return (
          <div className="space-y-3">
            <div>
              ...
              />
              {lecture.documentUrl && (
                <p className="text-sm text-red-600 mt-1">✓ Document selected: {lecture.document?.name}</p>
              )}
            </div>
          </div>
        );
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0 w-full sm:w-auto">
            ...
            <button
              ...
              disabled={processing}
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 sm:px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base font-medium"
            >
